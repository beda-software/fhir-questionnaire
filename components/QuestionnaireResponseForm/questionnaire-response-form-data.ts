...
export function questionnaireIdLoader(
  questionnaireId: string
): QuestionnaireIdLoader {
  return {
    type: "id",
...
**/
async function loadQuestionnaireResponseFormData(
  ...
  if (initialQuestionnaireResponse?.id) {
    populateRemoteData = success(
      initialQuestionnaireResponse as QuestionnaireResponse
    );
  } else {
    ...
    if (isSuccess(populateRemoteData) && autosave) {
      ...
      populateRemoteData = await serviceProvider.saveFHIRResource(
...
export async function handleFormDataSave(
  ...
  const { questionnaire, questionnaireResponse } =
    fromQuestionnaireResponseFormData(formData, {
      status: "completed",
  ...
  const saveQRRemoteData = await serviceProvider.saveFHIRResource(
    questionnaireResponse
  );
  ...
  return success({
    ...
    extractedBundle: isSuccess(extractRemoteData)
...
export function useQuestionnaireResponseFormData(
  ...
  const handleUpdate = async (
    ...
    const responseRemoteData = await props.serviceProvider.saveFHIRResource(
      draft.questionnaireResponse
    );
