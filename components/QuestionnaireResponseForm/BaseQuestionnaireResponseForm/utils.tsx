  ...
  questionnaireItems.forEach((item) => {
    ...
    if (item.type === "string" || item.type === "text") {
      ...
      schema = item.required
        ? yup.array().of(yup.mixed()).min(1).required()
        : yup.mixed().nullable();
    }
...
function getQuestionItemEnableWhenSchema(
  props: GetQuestionItemEnableWhenSchemaProps
) {
  return getEnableWhenItemsSchema({ ...props, currentIndex: 0 });
}

interface GetEnableWhenItemSchemaProps
  extends GetQuestionItemEnableWhenSchemaProps {
  currentIndex: number;
  prevConditionResults?: boolean[];
}

function getEnableWhenItemsSchema(
  props: GetEnableWhenItemSchemaProps
): yup.AnySchema {
  const {
    enableWhenItems,
    enableBehavior,
    currentIndex,
    schema,
    prevConditionResults,
  } = props;

  const { question, operator, ...enableWhenItem } =
    enableWhenItems[currentIndex]!;
  const answer = toAnswerValue(enableWhenItem, "answer")!;

  const isLastItem = currentIndex === enableWhenItems.length - 1;

  const conditionResults = prevConditionResults
    ? [...prevConditionResults]
    : [];
...
function getAnswerOptionsValues(
  answerOptionArray: QuestionnaireItemAnswerOption[]
): Array<AnswerValue> {
...
export function getFieldErrorMessage(
  ...
  const errorMessageWithInternalFieldName = fieldState.error.message.replace(
    /\[(\d+)\]/g,
    ".$1"
  );

  const errorMessageWithHumanReadableFieldName =
    errorMessageWithInternalFieldName.replace(field.name, text ?? "");
